...
class ListBrandProductAction implements RequestHandlerInterface
{
    private $logger;
    private $connection;
    ...
    {
        $this->logger->info('ListBrandProductAction: handler dispatched');
        ...
        try {
            ...
            $status = 1;
            $this->logger->info('ListBrandProductAction: --brand id---'.$brand_id);
            $db =  $this->connection;
            ...
            if(count($data)>0){
				...
				foreach ($planDatas as $key => $planData) {
                    ...
                    $this->logger->info('ListBrandProductAction: Number of products available for brand_id-'.$brand_id.'-is-'.$planData['id']);
                    ...
                    $promoSql = $db->prepare('SELECT * from map_plan_discount where plan_id=:product_plan_id and is_active=:status');
                    $promoSql->execute(array(':product_plan_id' => (int)$planData['id'],':status' => 1));
                    $promoCount += $promoSql->rowCount();
                ...
                $data['activePromoCount'] = $promoCount;
                ...
                $this->logger->info('ListBrandProductAction: Products not available for brand_id-'.$brand_id);
                $response->getBody()->write(
                    json_encode(array(
                        "code" => 1,
                        "status" => 2,
            ...
            $this->logger->info('ListBrandProductAction: Error in fetching all products for brand_id-'.$brand_id.'----'.$e->getMessage());
            $response->getBody()->write(
                json_encode(array(
                    "code" => 0,
                    "status" => 0,
